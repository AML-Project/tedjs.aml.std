...
(function() {
            ...
            controlers: {
                ...
                function: function() {
                    ...
                    for (var i = 0; i < childAml.length; i++) {
                        ...
                        controlers.sub[name] = aml.module(name);
                    }
    ...
    aml = {
        ...
        varExist: function(f, el) {
            if (typeof el === typeof undefined || !(tedApi.isElement(el))) return
            false;
        ...
        varValue: function(n, el) {
            if (typeof n === typeof undefined || typeof el === typeof undefined || !(tedApi.isElement(el))) return
            undefined;
            while (el) {
                ...
                if (G != -1) {
                    for (var GL = 0; GL < G.length; GL++) {
                        if (G[GL].left == n) {
                            try {
                                return
                                tedApi.run.call(el, "return " + G[GL].right);
                            } catch (e) {
                                return
                                G[GL].right
                            }
    ...
    ted.creatAttr("repeat", function(content) {
        ...
        if (reg_to.test(content)) {
            ...
            tedApi.each(obj, function(_Key) {
                if (tedApi.isFunction(this)) return
                0;
                if (i == 0) {
